...
# @File: PromptDelayedSignalSelectionDelayFirst.py
...
class PromptDelaySelection:
    ...
    def DoSelection(self, multiplicity_cut=False):
        ...
        v_IBDp_source = np.ones(len(self.df_map_with_filter)) * -1

        for index, row in tqdm.tqdm(
                self.df_map_with_filter[((self.df_map_with_filter["recE"]<self.Ed_cut[1])&
                                        (self.df_map_with_filter["recE"]>self.Ed_cut[0]))|
                                        ((self.df_map_with_filter["recE"]<self.Ed_cut2[1]) &
                                         (self.df_map_with_filter["recE"]>self.Ed_cut2[0]))].iterrows()):
            R_evt = GetVertexR(row)

            if (v_IBDp_tag[index]==1) :
                v_IBDd_tag[index] = 0
                continue

            index_time = (row["TriggerTime"]-self.df_map_with_filter["TriggerTime"]>self.t_cut[0]) & (row["TriggerTime"]-self.df_map_with_filter["TriggerTime"]<self.t_cut[1])
            ...
            index_filter_tagged = (v_IBDd_tag!=1) | (v_IBDp_tag!=1)
            index_E_prompt = ((self.df_map_with_filter["recE"]<self.Ep_cut[1]) & (self.df_map_with_filter["recE"]>self.Ep_cut[0]))
            index_FV_cut = (self.df_map_with_filter["R"]<self.R_FV_cut[1])&(self.df_map_with_filter["R"]>self.R_FV_cut[0])

            index_prompt_signal = (index_time& index_E_prompt & index_dR & index_filter_tagged & index_FV_cut)

            if multiplicity_cut:
                if Counter(index_prompt_signal==True)[True]==1:
                    v_IBDd_tag[index] = 1
                    v_IBDp_tag[np.where(index_prompt_signal)[0][0]] = 1
                    v_IBDp_source[np.where(index_prompt_signal)[0][0]] = self.df_map_with_filter["evtID"][index]

                else:
                    v_IBDd_tag[index] = 0
            else:
                if any(index_prompt_signal):
                    v_IBDd_tag[index] = 1
                    v_IBDp_tag[np.where(index_prompt_signal)[0][0]] = 1
                    v_IBDp_source[np.where(index_prompt_signal)[0][0]] = self.df_map_with_filter["evtID"][index]
        ...
        self.df_map_with_filter[self.name_source_delay] = np.array(v_IBDp_source, dtype=np.int32)

        self.df_map_with_new_tag = pd.concat(
            (self.df_map, self.df_map_with_filter.set_index("evtID")[self.name_tag_prompt].replace({-1:0}),
    ...
    def IsolationSelection(self):
        ...
        self.df_map_with_new_tag = pd.concat(
            (self.df_map, self.df_map_with_filter.set_index("evtID")[self.name_tag_isolation]),
            axis=1).fillna(0)
        return self.df_map_with_new_tag
    ...
    def SetDataset(self, df_evts: pd.DataFrame, index_filter: np.ndarray):
        ...
        if "R" not in self.df_map_with_filter.columns:
            from PlotDetectorGeometry import GetR3_XYZ
            self.df_map_with_filter["R"] = GetR3_XYZ( self.df_map_with_filter["recX"],
                                                      self.df_map_with_filter["recY"],
                                                      self.df_map_with_filter["recZ"] )**(1/3)/1e3
    ...
    def EvaluateTagging(self, name_truth_prompt="IBDp", name_truth_delay="IBDd"):
        ...
        df_map_IBDp_gf = self.df_map_with_new_tag[index_FV_cut].groupby([self.name_tag_prompt, "evtType"]).size()
        df_map_IBDd_gf = self.df_map_with_new_tag.groupby([self.name_tag_delay, "evtType"]).size()
        display(df_map_IBDp_gf)
        display(df_map_IBDd_gf)

        n_total_IBDp = self.df_map_with_new_tag[index_FV_cut].groupby("evtType").size().loc[name_truth_prompt]
        n_total_IBDd = self.df_map_with_new_tag.groupby("evtType").size().loc[name_truth_delay]

        print("IBD Prompt Signal Selection Efficiency:\t",
              df_map_IBDp_gf.xs(1, level=self.name_tag_prompt).loc[name_truth_prompt] / n_total_IBDp,
              f"{df_map_IBDp_gf.xs(1, level=self.name_tag_prompt).loc[name_truth_prompt]}/{n_total_IBDp}")
        print("IBD Delay  Signal Selection Efficiency:\t",
              df_map_IBDd_gf.xs(1, level=self.name_tag_delay).loc[name_truth_delay] / n_total_IBDd,
              f"{df_map_IBDd_gf.xs(1, level=self.name_tag_delay).loc[name_truth_delay]}/{n_total_IBDd}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Firstly Select Delayed Signal")
    ...
    parser.add_argument("--IBDSelectionAfterPSD", action="store_true", default=False,
                        help="Prompt-Delayed Selection After PSD")
    parser.add_argument("--MultiplicityCut", action="store_true", default=False,
                        help="Add Multiplicity Cut for Prompt-Delayed Selection")
