...
job={}
chmod 755 $job
hep_sub $job -e /dev/null -o /dev/null -wt short
"""
        
    def SetTemplate(self, template):
        self.template_detsim:str = template

    def GenDetsimScripts(self,v_momentum:np.ndarray=np.zeros(3), v_position:np.ndarray=np.zeros(3), name_particle:str="neutron", name_label:str="", name_dir_save="./"):
    ...
    def AddJobToSubDetsimScript(self, name_dir_save, name_job):
        if self.i_added_sub_job==0:
            ...
            option = "a"
        with open(f"{name_dir_save}/sub.sh", option) as f:
            f.write(self.template_sub_scripts.format(name_job))
