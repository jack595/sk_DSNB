...
def GetBuffer(v_triggerTime, i_center_event:int, length_buffer:float, return_index=True):
    """

    :param v_triggerTime: array of trigger time sequences
    :param i_center_event: choose which event to be the center of buffer
    :param length_buffer:  time length of buffer / ns
    ...
    for j in range(  i_center_event+1):
        if i_center_event-j<0 or v_triggerTime[i_center_event]-v_triggerTime[i_center_event-j]>length_buffer or \
        v_triggerTime[i_center_event] - v_triggerTime[i_center_event - j] < 0 :
            break
