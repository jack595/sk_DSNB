...
class TrainTool:
    def __init__(self):
        ...
        self.input_train = []
        self.target_train = []
        self.map_tag_particles = {"alpha":0, "e-":1}
        self.key_tag = "tag"
        ...
        self.filter = {"Erec": [0, 1.5], "R3": [0, 4096]}

        # Training Settings
        self.normalized_input=False
        self.strategy = "Combine"
        self.path_model = "./"
        self.name_file_model = self.path_model+"model_LowE.pkl"
    ...
    def GetEfficiencySigma(self, eff):
        ...
        relative_sigma_eff = np.sqrt(eff*(1-eff)*len(self.input_test[self.target_test==1]))/len(self.input_test[self.target_test==1])
